...
## Amendment: 2026-09-13, owner-directed completion

The owner's later instructions add Updates as a fifth destination, with recent history, a manual check, important opening notices and expiring item/time pattern notices during a visible session. These describe aggregate sales; they do not identify or predict an individual customer's return.

The current provisional public plans are $39/month for one active location and $99/month for up to three. Legacy agreements remain intact. `PRICING_DECISION.md` is the current pricing decision and includes the $3 inference assumption, making illustrative variable cost $22.60 per location. None of those usage costs or price-conversion outcomes are measured production results.

The owner has not finalized the production model. The existing configurable `claude-opus-5` default remains an implementation default, not a final model-selection decision or a verified superiority claim. Official standard API pricing is linked from `quantify_app/budget.py`; actual invoices can differ by provider, cache mode and discounts.

The current $6 check is a spend guard based on recorded completed calls. It is not a guaranteed provider invoice cap: concurrent calls, billable failures, missing usage, unknown-model pricing and estimation error can exceed it. A hard cap would require atomic reservations and reconciliation with provider usage. Provider-side spend limits should be configured before live paid usage. The deterministic writer remains available at a guard limit. Updates and composition calls now explicitly attribute their usage to the location. Requests have a 45-second transport timeout and no automatic SDK retry.

Personalization must distinguish an official statutory reference from an employer's actual cost. No combined payroll burden can be inferred from a city alone. Costs and retained-money figures remain unknown until required actual wage and burden inputs exist. Geographic context requires a matching place and source. These specific rules supersede earlier screen, pricing and model-selection descriptions in this document.
