"""What the writing layer is allowed to cost, per location, per month.

Quantify computes every number itself and the model only writes sentences about
them. That work is cached: one brief per location per day, keyed by a fingerprint
of the record it was written from. While the fingerprint holds, a location costs
about three dollars a month against seventy nine dollars of revenue.

Two paths in the product deliberately skip that cache, because an operator who
has just corrected something needs to see the correction. A refresh on the day
narrative and a refresh on an item both pass `force=True` straight through to the
model. Neither had a limit. A stuck browser tab, a retry loop, or one frustrated
owner leaning on a button could run the bill with nothing counting and nothing
stopping it.

This module counts and stops. What makes that safe is the local writer: every
task in `ai.py` carries a deterministic writer built from the same record, so a
location that runs out of budget does not lose its brief. It gets the written one
instead of the model one, and the difference is prose quality, not information.

So the rule is: a customer who behaves badly degrades to the deterministic
product. A customer is never cut off, and nobody is ever surprised by an invoice.

Prices are dollars per million tokens, read from the Anthropic price list on
13 September 2026. A cache read costs a tenth of an input token and a cache write
costs a quarter more than one. A test fails when PRICES_REVIEWED is over a year
old, the same way the minimum wage table in costs.py does.
"""
...
PRICES_SOURCE = "Anthropic published API price list, read 13 September 2026"
